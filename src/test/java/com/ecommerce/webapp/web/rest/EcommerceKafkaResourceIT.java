...
import com.ecommerce.webapp.config.KafkaProperties;
...
class EcommerceKafkaResourceIT {
    ...
    void setup() {
        ...
        EcommerceKafkaResource kafkaResource = new EcommerceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ecommerce-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ecommerce-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
